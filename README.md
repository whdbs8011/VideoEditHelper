...
스토리보드 CSV를 검증하고 미디어를 찾아 Adobe Premiere Pro 타임라인에 자동 배치합니다.
...
5. CSV와 미디어 루트 폴더를 선택한 뒤 **타임라인 생성**을 누릅니다.
...
- `--dry-run`: CSV/미디어 검증과 JSX 생성만 수행
...
csv_parser.py             CSV 검증, 시간 파싱, 미디어 탐색
...
- `track_index`가 실제 시퀀스 트랙 범위를 벗어나면 해당 CSV 행 번호와 함께 중단합니다.
