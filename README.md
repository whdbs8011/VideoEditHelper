...
미디어 이름은 정확히 일치하는 파일을 먼저 사용합니다. 예를 들어 컷 번호가 `24`일 때 `24.mp4`가 없고 `24-1.mp4`, `24-2.mp4`가 있으면 숫자가 가장 높은 `24-2.mp4`를 자동 선택합니다. `24.mp4`가 존재하면 테이크 파일보다 우선합니다. 같은 최고 테이크 번호의 후보가 여러 폴더나 확장자로 중복되면 임의 선택하지 않고 해당 행을 건너뜁니다.

미디어를 찾지 못하거나 후보가 여러 개라 하나를 안전하게 결정할 수 없는 행은 전체 작업을 중단하지 않고 건너뜁니다. 각 행은 절대 `start_time`에 배치되므로 건너뛴 컷의 시간 구간은 빈 상태로 유지됩니다. 완료 화면의 `빈 구간` 수와 경고 목록에서 해당 행을 확인할 수 있습니다.
